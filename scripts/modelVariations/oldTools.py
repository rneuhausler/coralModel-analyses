...
def initialFinal3(types=[], C=[], CN=[], CNr=[], simulation=1, 
                 timesteps=[0,1,2]):
    ...
    plt.subplot(131).set_title('-C-  ' + str(C[timesteps[0],simulation])+
                              ...
                              fontweight="bold")
    plt.imshow(shaper(types[timesteps[0],:],10), 
               cmap=cmap, norm=norm)
    ...
    plt.subplot(132).set_title('-C-  ' + str(C[timesteps[1],simulation])+
                              ...
                              fontweight="bold")
    plt.imshow(shaper(types[timesteps[1],:],10), 
               cmap=cmap, norm=norm)
    ...
    plt.subplot(133).set_title('-C-  ' + str(C[timesteps[2],simulation])+
                              ...
                              fontweight="bold")
    plt.imshow(shaper(types[timesteps[2],:],10), 
               cmap=cmap, norm=norm)  
    plt.show()
...
def tda_prep(df, rows, fineness=2, refined_grid=False):
    df = binary(df)
    dfi = 1-df
...
def plotTDA(data, tdaInput, distanceType, including, run, simulation, grazing):
    
    grid = "grid" + str(tdaInput.shape)

    dgm = lower_star_img(tdaInput) 
    ...
    plt.title(distanceType + " Distance, " + including
              + ", " + grid, fontsize=12, fontweight = 'bold')
    
    plt.xlabel("\n"+ "\n" + ' Grazing ' + '.' + str(grazing)
               + ' Sim '+str(simulation) + ' Step ' + str(run),
    ...
    plt.savefig("write-up-1.5/"+ distanceType + including + 
                str(simulation) + 'Step' + str(run) + 
                "Grazing"+str(grazing))
