...
def densityExtract(Moorea, Node, Count):
    if Count == 0:
        neighbors = 0
    else:
...
def densityExtractSpecific(Moorea, NodeType, NeighborType, Count):
    if Count == 0:
        neighbors = 0
    else:
        neighbors = np.array([Moorea.nodes[n].density/
                              Moorea.nodes[n].density.sum()
                              for n,val in enumerate(Moorea.nodes)
                              if Moorea.nodes[n].type == NodeType]).mean(axis=0)[NeighborType]
    return(neighbors)
...
def viewRuns(topDirectory):
    ...
    files = [path+'/'+file for path, d, f in os.walk(topDirectory) for
...
def viewImages(topDirectory, overviewOfRuns):
    ...
    ## Assign column names + create file index
    overviewOfImages.columns = ['Rows', 'Columns', 'Grid Option', 'Grazing', 'Simulation', 'Timestep','Initial Coral Percent', 'Initial Macroalgae Percent', 'r', 'd', 'a', 'y', 'Time', 'Record Rate', 'Number of Simulations']
...
def viewRuns2(topDirectory):
    ...
    ## Assign column names + create file index
    overviewOfRuns.columns = ['Rows', 'Columns', 'Grid Option', 'Grazing', 'Initial Coral Percent', 'Initial Macroalgae Percent', 'r', 'd', 'a', 'y', 'Time', 'Record Rate', 'Number of Simulations', 'Threshold']
