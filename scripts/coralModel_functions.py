...
def pull_info(Moorea, simulation, timestep, image_counter=image_counter):
    ...
    coral_neighbors = round(tl.extract_neighbors(Moorea, int(0), C), 2)
    turf_count = np.count_nonzero(image==1)
    turf_neighbors = round(tl.extract_neighbors(Moorea, int(1), T), 2)
    macroalgae = np.count_nonzero(image==2)
    macroalgae_count = round(tl.extract_neighbors(Moorea, int(2), M), 2)
...
if __name__ == '__main__':
    ...
    path='./output/'+str(number_of_rows)+'x'+str(number_of_columns)+
         '/grid'+str(grid_option)+'/grazing'+str(int(g*100))+'/threshold'+str(int(neighborhood_threshold*100))+
         '/'
